...
# RENAMED FUNCTION to indicate use in a FedProx system. 
# The aggregation mathematics remains the weighted average of FedAvg.
def fedprox_aggregator(state_dicts: List[Dict[str, torch.Tensor]], weights: Optional[List[float]] = None) -> Dict[str, torch.Tensor]:
    ...
    if w_sum <= 0 or any(math.isnan(w) or math.isinf(w) for w in weights):
        raise ValueError("Invalid FedProx weights.")
    out = {}
    keys = klist[0]
    for k in keys:
        acc = None
        for sd, w in zip(state_dicts, weights):
            ...
            # The aggregation is still a weighted average of client weights
...
def main():
    ap = argparse.ArgumentParser(description="FedProx aggregator for DenseNet121 CheXpert (torchvision).")
    ...
    ap.add_argument("--weights", help="Comma-separated sample counts to weight aggregation (e.g. '1024,980,765,1200').")
    ap.add_argument("--manifests", nargs="*", help="Optional JSON manifests per client with {'num_samples': int}.")
    ap.add_argument("--mu", type=float, default=0.0, help="FedProx proximal term weight (mu). Only used for logging/config.")
    ...
    # Determine FedProx weights (sample counts are used for weighted averaging)
    ...
    # Aggregate using the FedProx aggregator (which performs FedAvg)
    global_sd = fedprox_aggregator(sds, w)
